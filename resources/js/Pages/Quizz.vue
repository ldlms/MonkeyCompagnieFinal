<template>
    <section>
        ...
        <div class="mainQuizz">
            <div v-for="(quizz, key) in quizzs" :key="key" v-show="show === key">
                ...
                <div v-if="quizz.quizzable_type === 'Qcm'" class="containerQcm">
                    ...
                    <div class="reponseQcm" :class="{ selected: isSelected('A') }" @click="selectionnerChoix('A')"> A.
                        {{ quizz.quizzable.choix1 }}</div>
                    <div class="reponseQcm" :class="{ selected: isSelected('B') }" @click="selectionnerChoix('B')"> B.
                        {{ quizz.quizzable.choix2 }}</div>
                    <div class="reponseQcm" :class="{ selected: isSelected('C') }" @click="selectionnerChoix('C')"> C.
                        {{ quizz.quizzable.choix3 }}</div>
                    <div class="reponseQcm" :class="{ selected: isSelected('D') }" @click="selectionnerChoix('D')"> D.
                        {{ quizz.quizzable.choix4 }}</div>
                    <button @click="checkReponse">Vérifier</button>
                </div>
                <div v-if="quizz.quizzable_type === 'Vraifaux'" class="containerVf">
                    <div class="titreVf">Vrai ou faux ?</div>
                    <div class="question"> {{ quizz.question }} </div>
            ...
            <div v-if="show >= quizzs.length" class="resultatQuizz">
                ...
                <p v-if="points / quizzs.length == 1">Felicitation ! Venez mesurer vos connaissances aux autres lors de
                    nos évènements !</p>
...
.selected {
    color: green;
    /* Ou la couleur que vous préférez */
}
